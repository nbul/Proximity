...
%     Ske = zeros(size(B,1),1);
%     Bor = zeros(size(B,1),1);
%     %begins the second loop (loop within a loop)
%     for m = 1:size(B,1)
%         image_temp = IC4 .* 0;
%         image_temp(B(m,1), B(m,2)) = 1;
%         image_temp = imdilate(image_temp, strel('disk', 4,0));
%         cc = bwconncomp(image_temp);
%         s_Ske = regionprops(cc, SignalSke, 'MeanIntensity');
%         s_Bor = regionprops(cc, double(SignalBor), 'MeanIntensity');
%         Ske(m) = s_Ske.MeanIntensity;
%         Bor(m) = s_Bor.MeanIntensity;
%  
%     C = [Bor, Ske];
%     C(any(isnan(C), 2), :) = [];
%     [rho,pval] = corr(C);
    
    Puncta = imbinarize(imadjust(SignalBor), 2*graythresh(imadjust(SignalBor)));
    ...
    summary(k,:) = [k, Signal_Ske_P, Signal_Ske_nonP, Signal_Ske_halo, Signal_Ske_cyto, Signal_Bor_P, Signal_Bor_nonP, Signal_Bor_halo, Signal_Bor_cyto];
...
all.Properties.VariableNames = {'Wing', 'Tubulin_puncta', 'Tubulin_nopuncta', 'Tubulin_puncta_halo', 'Tubulin_cyto', 'Ds_puncta', 'Ds_nopuncta', 'Ds_puncta_halo', 'Ds_cyto'};
